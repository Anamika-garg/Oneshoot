...
import useNotifications from "@/hooks/useNotifications"; // Importing hook for notifications

const ProfilePage = () => {
  const supabase = createClient();
  const [user, setUser] = useState(null);
  const [loading, setLoading] = useState(true);
  const [unreadCount, setUnreadCount] = useState(0); // Store unread notifications count
      ...
      <Tabs defaultValue='overview' className='w-full mt-16 z-20 relative'>
        <TabsList className='grid w-full grid-cols-4 mb-8 bg-[#0E0E0E]'>
          ...
          >
            ...
            {unreadCount > 0 && (
              <span className='  bg-red-500 text-white text-xs rounded-full px-2 py-1 w-6 h-6'>
                {unreadCount}
              </span>
            )}
          </TabsTrigger>
